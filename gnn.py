...
def FeaturePropagation_zero(x: Tensor):
        x=x.clone()
        pressure, node_type = x[:,0], x[:,1] 
        pressure[node_type == 0] = 0
        x[:,0] = pressure
...
def FeaturePropagation_average(x: Tensor, batch: Tensor):
    ...
    for graph_id in range(num_graphs):
        mask_field = (batch == graph_id) & (node_type == 0)  # Field Node mask
        mask_surface = (batch == graph_id) & (node_type == 1)  # Surface Node mask

        if mask_surface.sum() > 0:  # Calculating average only when surface nodes exist
            mean_pressure = pressure[mask_surface].mean()  # pressure average of Surface nodes
            pressure[mask_field] = mean_pressure  # allocate pressure to Field nodes
...
def FeaturePropagation_average_test(x: Tensor):
    
    x = x.clone()
    pressure, node_type = x[:,0], x[:,1] 


    mask_field = node_type == 0  # Field Node mask
    mask_surface = node_type == 1  # Surface Node mask
...
class Multiscale_MessagePassing_UNet(torch.nn.Module):
    def __init__(self, 
        ...
        super().__init__()
        self.in_channels_node = in_channels_node  # 5 = 3 (node features: pressure, node type, distance) + 2 (global features: u_inf, v_inf, AOA)
        self.out_channels_node = out_channels_node # 3 (pressure, velocity)
        self.in_channels_edge = in_channels_edge # 4 (rx, ry, l, ld)
